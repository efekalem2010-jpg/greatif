...
# --- GÖMÜLÜ SUNUCU VE GÜNCELLENMİŞ KAZIMA MOTORU ---
                ...
                if platform == 'X (Twitter)':
                    ...
                    if tweet_id_match:
                        ...
                        with urllib.request.urlopen(req, timeout=10) as response:
                            ...
                            result["views"] = str(views_data.get("count", "Gizli / Yok")) if views_data else "Yok"
                    ...
                    with urllib.request.urlopen(req, timeout=10) as response:
                        ...
                        combined_text = " ".join(meta_matches)
                        
                        title_match = re.search(r'<title[^>]*>([^<]+)</title>', html, re.IGNORECASE)
                        if title_match:
                            combined_text += " " + title_match.group(1)

                        likes_match = re.search(r'([\d,\.]+[KkMm]?)\s+(?:Likes|Beğeni)', combined_text, re.IGNORECASE)
                        comments_match = re.search(r'([\d,\.]+[KkMm]?)\s+(?:Comments|Yorum)', combined_text, re.IGNORECASE)
                        views_match = re.search(r'([\d,\.]+[KkMm]?)\s+(?:Views|Görüntüleme|İzlenme)', combined_text, re.IGNORECASE)

                        if platform == 'Instagram':
                            user_match = re.search(r'\(@([a-zA-Z0-9_\.]+)\)', combined_text)
                            if user_match: 
                                result["accountName"] = "@" + user_match.group(1)
                            elif title_match and '(@' in title_match.group(1):
                        ...
                        if result["likes"] == "Veri Yok" and result["comments"] == "Veri Yok":
                             self._send_json({"error": f"{platform} bu bağlantı için bot koruması uyguladı ve verileri gizledi."}, 400)
                             return
